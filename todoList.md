...
* how to build android app in container. specify sdk version for app
  ...
  * a gulp task monitor the java code and when they changes, run the ``gradle assembleDebug`` command; ``gradle assembleRelease`` for production, **run ``gradle lint`` to verify any configuration error**
